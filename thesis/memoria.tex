...
As this problem only addresses only one type of weed, the cardilla, the usage of only one class was needed in order to label the entire dataset. This by itself can cause future problems when predicting instances.

Initially, the dataset contained mainly images full or samples of the plant, but not of the ground without any weed. This could cause an increase in false positives, as the neural network could predict that \textit{any soil with any type of grass is a cardilla}. Data augmentation and manually extracting those frames was needed in order to use them training and reducing false-positives.


\subsubsection{Labelling Criteria}

After the extensive research commented in the section \ref{subsec:imageAnalysis}, it is possible to start with the image labelling. This is the most time-consuming part of the dataset creation. Identifying  all possible samples from the dataset manually is complicated without the help of a team specialised in the topic. Knowing this, it is probable that the dataset labelling may contain some errors that may cause the appereance of false positives or reduced performance of our model.
...
Para el dataset genérico con todo tipo de información menos hiperspectral:

\begin{itemize}
	\item \textbf{El mejor modelo fue yolo11s\_e60\_b32\_s42\_box7.5\_7821d con 0.632 de precisión}. Con respecto a su homólogo en el dataset genérico, obtenemos un 6\% más (o.567)
	\item Mantenemos recall.
\end{itemize}

Se puede afirmar que, con una menor altura, obtenemos una mayor precisión, pero una peor estimación del entorno global.

\subsection{Results}

Para el dataset que se basa en vuelo rasante:

...
MODEL_ROUTE = 'results_yolo_tiled_v1/yolov8n_e200_b16_s42_box5.0_f203a/weights/best.pt'
...
def merge_adjacent_boxes(boxes, scores, classes, margin=5):
    ...
    return torch.stack(merged), torch.tensor(merged_scores), torch.tensor(merged_classes)
...
def are_adjacent(box1, box2, margin=5):
    ...
    horizontal_close = (
        abs(x2_1 - x1_2) <= margin or  # right border box1 close left box2
        abs(x2_2 - x1_1) <= margin     # right border box2 close left box1
    )
    
    vertical_close = (
        abs(y2_1 - y1_2) <= margin or  # bottom border box1 close upper box2
        abs(y2_2 - y1_1) <= margin     # bottom border box2 close upper box1
    )
    
    horizontal_overlap = not (x2_1 < x1_2 - margin or x2_2 < x1_1 - margin)
    vertical_overlap = not (y2_1 < y1_2 - margin or y2_2 < y1_1 - margin)
    
    return (horizontal_close and vertical_overlap) or (vertical_close and horizontal_overlap)
        ...
        for x in range(0, w, grid_size):
            ...
            if grid.shape[0] < grid_size or grid.shape[1] < grid_size:
                padded = cv2.copyMakeBorder(
                    grid,
                    0, grid_size - grid.shape[0],
                    0, grid_size - grid.shape[1],
                    cv2.BORDER_CONSTANT,
                    value=[0, 0, 0]
                )
                grids.append(padded)
...
def process_frame_with_grids(frame, model, conf_threshold, save_debug=False):
    ...
    for idx, (r, (ox, oy)) in enumerate(zip(batch_results, offsets)):
        ...
        for box, score, cls in zip(boxes, scores, classes):
            ...
            if save_debug:
                cv2.rectangle(grid_with_boxes, 
                             (int(x1), int(y1)), 
                             (int(x2), int(y2)), 
                             (0, 255, 0), 2)
                cv2.putText(grid_with_boxes, 
                           f"{score:.2f}", 
                           (int(x1), int(y1)-5),
...
def main():
    ...
    if not IMAGE_TO_PREDICT.lower().endswith('.mp4'):
        ...
        boxes, scores, classes = process_frame_with_grids(img, model, CONF_THRESHOLD, save_debug=True)
    ...
    # If it's a video → process frame by frame
        ...
        while cap.isOpened():
            ...
            boxes, scores, classes = process_frame_with_grids(frame, model, CONF_THRESHOLD)
